...
# GPIO Pins
GPIO_PIN_SERVO = 12  # Servo motor control pin
TRIGGER_PIN = 17  # Trigger pin for HC-SR04
ECHO_PIN = 27  # Echo pin for HC-SR04
...
def read_hx711_count() -> int:
    """
    Read raw data from the HX711 load sensor.
    """
...
def run_system() -> None:
    try:
        ...
        while True:
            ...
            if distance is not None and distance < 20 and load_weight > 0:
                print("Bottle detected! Rotating servo...")
                run_servo()
                time.sleep(1)
            else:
